...
function Allproducts() {
    ...
    const vegetables = filteredAndSortedProducts.filter(item => item.category === 'Vegetables');
    const leafyVegetables = filteredAndSortedProducts.filter(item => item.category === 'Leafy Vegetables');
        ...
        return (
            <motion.div
                ...
                className={`group cursor-pointer ${
                    viewMode === 'list'
                        ? 'flex bg-gradient-to-br from-white to-green-50 rounded-3xl shadow-xl border border-green-100 hover:shadow-2xl hover:scale-[1.01] transition-all duration-300 p-4 items-center gap-6 w-full min-h-[160px]'
                        : ''
                }`}
            ...
            >
                {viewMode === 'grid' ? (
                    ...
                    <div className="bg-gradient-to-br from-white to-green-50 rounded-3xl shadow-xl border border-green-100 hover:shadow-2xl hover:scale-105 hover:border-green-400 transition-all duration-300 overflow-hidden relative">
                        ...
                        <div className="relative pt-[100%] overflow-hidden flex items-center justify-center">
                            ...
                            <button
                                ...
                                className={`absolute bottom-3 left-3 p-2 rounded-full transition-all duration-300 shadow-lg ${
                                    isFavorite 
                    ...
                    // Improved List View Card
                    <>
                        {/* Image */}
                        <div className="relative flex-shrink-0 w-36 h-36 rounded-2xl overflow-hidden flex items-center justify-center bg-gray-50">
                            <img
                            ...
                            {/* Discount Badge */}
                            {discountPercentage > 0 && (
                                <div className="absolute top-2 right-2 bg-gradient-to-r from-pink-400 to-green-400 text-white px-2 py-1 rounded-full text-xs font-bold shadow-lg">
                                    {discountPercentage}% OFF
                                </div>
                            )}
                            {/* Best Seller Badge */}
                            {isBestSeller && (
                                <div className="absolute top-2 left-2 bg-gradient-to-r from-yellow-400 to-pink-400 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg z-10">
                                    Best Seller
                                </div>
                            )}
                            {/* Favorite Button */}
                            <button
                                ...
                                className={`absolute bottom-2 left-2 p-2 rounded-full transition-all duration-300 shadow-lg ${
                                    isFavorite 
                                        ? 'bg-red-500 text-white' 
                                        : 'bg-white/90 backdrop-blur-sm hover:bg-white'
                                }`}
                            ...
                            {/* Stock Indicator */}
                            <span className={`absolute bottom-2 right-2 w-3 h-3 rounded-full ${inStock ? 'bg-green-500' : 'bg-red-500'} border-2 border-white`} title={inStock ? 'In Stock' : 'Out of Stock'}></span>
                        </div>
                        {/* Content */}
                        <div className="flex-1 flex flex-col justify-between h-full min-h-[120px] pl-2">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4 mb-2">
                                <span className={`px-2 py-1 rounded-full text-xs font-bold mb-2 sm:mb-0 ${
                                    category === 'Vegetables' ? 'bg-green-100 text-green-700' : 'bg-pink-100 text-pink-700'
                                }`}>
                                    {category}
                                </span>
                                <h1 className="text-lg font-bold text-gray-900 line-clamp-2 flex-1 leading-tight">{title}</h1>
                            </div>
                            <div className="flex items-end justify-between flex-wrap gap-2 mb-2">
                                ...
                                className="px-6 py-2 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-full flex items-center gap-2 font-semibold shadow-lg transition-all hover:from-green-600 hover:to-green-700 hover:scale-105 focus:ring-2 focus:ring-green-400"
                                >
                                <FiShoppingCart className="w-5 h-5" />
                                    Add To Cart
                                </button>
